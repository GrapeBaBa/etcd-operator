...
func (c *Cluster) deleteClientServiceLB() error {
	err := c.config.KubeCli.CoreV1().Services(c.cluster.Namespace).Delete(c.cluster.Name, nil)
	if err != nil {
...
func (c *Cluster) removePodAndService(name string) error {
	err := c.config.KubeCli.CoreV1().Services(c.cluster.Namespace).Delete(name, nil)
	if err != nil {
		if !k8sutil.IsKubernetesResourceNotFoundError(err) {
			return err
		}
	}
	err = c.config.KubeCli.CoreV1().Pods(c.cluster.Namespace).Delete(name, v1.NewDeleteOptions(0))
	if err != nil {
		if !k8sutil.IsKubernetesResourceNotFoundError(err) {
			return err
		}
...
func (c *Cluster) updateStatus() error {
	...
	newCluster, err := k8sutil.UpdateClusterTPRObject(c.config.KubeCli.CoreV1().RESTClient(), c.cluster.GetNamespace(), newCluster)
	if err != nil {
		return err
	}
