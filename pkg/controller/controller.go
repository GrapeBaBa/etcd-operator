...
func (c *Controller) monitor(watchVersion string) (<-chan *Event, <-chan error) {
	...
	go func() {
		...
		host := c.MasterHost
		ns := c.Namespace
		// If we want to use client-go and RESTClient to watch, we need to set codec.
		// TODO: switch to Operator client.
		httpcli := (c.KubeCli.CoreV1().RESTClient().(*rest.RESTClient)).Client
