...
func (gc *GC) FullyCollect() error {
	clusters, err := k8sutil.GetClusterList(gc.k8s.CoreV1().RESTClient(), gc.ns)
	if err != nil {
		return err
	}
...
func (gc *GC) collectResources(option v1.ListOptions, runningSet map[types.UID]bool) error {
...
func (gc *GC) collectPods(option v1.ListOptions, runningSet map[types.UID]bool) error {
	pods, err := gc.k8s.CoreV1().Pods(gc.ns).List(option)
	if err != nil {
		return err
	}
	...
	for _, p := range pods.Items {
		...
		if !runningSet[p.OwnerReferences[0].UID] {
			err = gc.k8s.CoreV1().Pods(gc.ns).Delete(p.GetName(), v1.NewDeleteOptions(0))
			if err != nil {
				return err
			}
...
func (gc *GC) collectServices(option v1.ListOptions, runningSet map[types.UID]bool) error {
	srvs, err := gc.k8s.CoreV1().Services(gc.ns).List(option)
	if err != nil {
		return err
	}
	...
	for _, srv := range srvs.Items {
		...
		if !runningSet[srv.OwnerReferences[0].UID] {
			err = gc.k8s.CoreV1().Services(gc.ns).Delete(srv.GetName(), nil)
			if err != nil {
				return err
			}
...
func (gc *GC) collectReplicaSet(option v1.ListOptions, runningSet map[types.UID]bool) error {
	rss, err := gc.k8s.ExtensionsV1beta1().ReplicaSets(gc.ns).List(option)
	if err != nil {
		return err
	}
	...
	for _, rs := range rss.Items {
		...
		if !runningSet[rs.OwnerReferences[0].UID] {
			err = gc.k8s.ExtensionsV1beta1().ReplicaSets(gc.ns).Delete(rs.GetName(), nil)
			if err != nil {
				return err
			}
