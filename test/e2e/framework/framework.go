...
func Teardown() error {
	...
	if err := Global.KubeClient.CoreV1().Pods(Global.Namespace).Delete("etcd-operator", v1.NewDeleteOptions(10)); err != nil {
		return err
	}
	Global = nil
...
func (f *Framework) setupEtcdOperator(opImage string) error {
	...
	err = k8sutil.WaitEtcdTPRReady(f.KubeClient.CoreV1().RESTClient(), 5*time.Second, 60*time.Second, f.Namespace)
	if err != nil {
		return err
	}
